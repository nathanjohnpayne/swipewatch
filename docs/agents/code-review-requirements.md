...
> Canonical source: `mergepath/docs/agents/code-review-requirements.md`. This file is propagated verbatim to consumer repos via the propagation manifest; edit it at the canonical source, never in a consumer copy. It is a pointer and nothing else: every file it names (`REVIEW_POLICY.md`, `.github/review-policy.yml`, `AGENTS.md`) exists in every repo that receives it, so the pointer resolves everywhere.
